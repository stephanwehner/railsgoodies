...
describe RailsGoodies::RdbPrompt::Sqlite3Prompt do
  it 'should inherit from AbstractPrompt' do
    RailsGoodies::RdbPrompt::Sqlite3Prompt.ancestors[1].should == RailsGoodies::RdbPrompt::AbstractPrompt
  end

  it 'should invoke exec with sqlite3 and database' do
    sqlite3_prompt = RailsGoodies::RdbPrompt::Sqlite3Prompt.new({'database'=>'testdb'}, {})
    sqlite3_prompt.should_receive(:exec).with('sqlite3 testdb')
    sqlite3_prompt.run
  end

  it 'should invoke exec with given executable and database' do
    sqlite3_prompt = RailsGoodies::RdbPrompt::Sqlite3Prompt.new({'database'=>'testdb'}, {:executable => 'testexecutable'})
    sqlite3_prompt.should_receive(:exec).with('testexecutable testdb')
    sqlite3_prompt.run
  end
end

describe RailsGoodies::RdbPrompt::MysqlPrompt, 'assembling my.cnf' do
  it 'should inherit from MysqlPrompt' do
    RailsGoodies::RdbPrompt::MysqlPrompt.ancestors[1].should == RailsGoodies::RdbPrompt::AbstractPrompt
  end

  it 'should assemble my.cnf from config' do
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'database'=>'testdb'}, {})
    mysql_prompt.get_my_cnf.should == "[client]\ndatabase=testdb"

    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'a'=>'1',
                                                        'b'=>'2',
                                                        'c'=>'3',
                                                        'd'=>'4',
                                                        'database'=>'required'}, {})
  ...
  it 'should not copy adpater to my.cnf' do
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'adapter' => 'mysql', 'database'=>'testdb'}, {})
    mysql_prompt.get_my_cnf.should == "[client]\ndatabase=testdb"
  end

  it 'should rename username to user in my.cnf' do
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'username' => 'test_un', 'database'=>'testdb'}, {})
    mysql_prompt.get_my_cnf.should == "[client]\ndatabase=testdb\nuser=test_un"
  end
  ...
  it 'should not copy ignore values to my.cnf' do 
    ...
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'a'=>'alphabet',
                                                        'longer'=>'2',
                                                        'x'=>'yz',
                                                        'd'=>'4',
...
describe RailsGoodies::RdbPrompt::MysqlPrompt, 'invoking run' do
  it 'should output my.cnf with mycnf_only option' do
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'database' => 'required', 'a'=>'1','b'=>'2'},{:mycnf_only => true})
  ...
  it 'should write my.cnf to the pipe-writer' do
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'database' => 'required', 'a'=>'1','b'=>'2'},{})
  ...
  it 'should invoke exec with given executable' do
    mysql_prompt = RailsGoodies::RdbPrompt::MysqlPrompt.new({'database' => 'required', 'a'=>'1','b'=>'2'},{:executable => 'testexec'})
