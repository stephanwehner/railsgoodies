...
describe RGoodies::RdbPrompt::AbstractPrompt do
  it 'should raise error when database is empty' do
    lambda {
      abstract_prompt = RGoodies::RdbPrompt::AbstractPrompt.new({}, {})
    }.should raise_error(RuntimeError, 'No database name found')
  end

  it 'should raise error when database is the empty string' do
    lambda {
      abstract_prompt = RGoodies::RdbPrompt::AbstractPrompt.new({'database' => ''}, {})
